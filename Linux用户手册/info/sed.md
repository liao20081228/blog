...
|**'s/REGEXP/REPLACEMENT/\[FLAGS\]'**|将正则表达式REGEXP与模式空间的内容匹配。如果找到，则用REPLACEMENT替换匹配的字符串。|
...
## 3.3 替换命令s
“s”命令可能是“sed”中最重要的命令，并且有很多不同的选项。 “s”命令的语法为's/REGEXP/REPLACEMENT/FLAGS'。

它的基本概念很简单：“s”命令尝试将模式空间与提供的正则表达式REGEXP相匹配；如果匹配成功，则匹配的模式空间部分替换为REPLACEMENT。

有关REGEXP语法的详细信息参见[正则表达式地址](#regexp_address)。

REPLACEMENT可以包含“\N”（N是1到9之间的数字）引用，该引用指的是匹配项中包含在第N个“\(”和“\)”之间的部分。另外，REPLACEMENT可以包含未转义的“＆”字符，表示引用模式空间的整个匹配部分。

在任何给定的“s”命令中，“/”字符可以统一替换为任何其他单个字符。仅当在“/”字符前面带有“\”字符时，它才能出现在REGEXP或REPLACEMENT中。

最后，作为GNU sed扩展，您可以使用由反斜杠和字母“L”，“l”，“U”，“u”或“E”中的一个组成的特殊序列。含义如下：

|序列|说明|
|:--|:--|
|“\L”|将替换项转换为小写，直到遇到“\U”或“\E”|
|“\l”|将下一个字符转为小写。|
|“\U”|将替换转换为大写，直到遇到“\L”或“\E”|
|“\u”|将下一个字符转为大写，|
|“\E”|停止以“\L”或“\U”开始的大小写转换。|

当使用'g'标志时，大小写转换不会从一次正则表达式传播到另一次。例如，在模式空间中使用“a-b-”执行以下命令时：
``` shell
	s/\(b\?\)-/\u\1x/g
```
输出为“axxB”。替换第一个“-”时，“\u”序列仅影响空替换“\1”。将“b-”替换为“xB”时，它不会影响添加到模式空间的“x”字符。

另一方面，如果“\l”和“\u”后面跟着一个空的替换，它们确实会影响替换文本的其余部分。在模式空间中使用“ a-b-”时，以下命令：
     s / \（b \？\）-/ \ u \ 1x / g

会将“-”替换为“ X”（大写），将“ b-”替换为“ Bx”。如果不希望出现这种情况，则可以通过在'\ 1'之后添加'\ E'序列来防止这种情况。

   要在最终替换中包含文字'\'，'＆'或换行符，请确保在替换中所需的'\'，'＆'或换行符之前加一个'\'。

   's'命令后可以跟随零个或多个以下标志：

'G'
     将替换项应用到REGEXP的_all_匹配项中，而不仅仅是第一个。

'数字'
     仅替换REGEXP的第NUMBER个匹配项。

     注意：POSIX标准未指定当您混合使用'g'和NUMBER修饰符时应该发生的情况，并且目前在'sed'实现中尚未达成共识。对于GNU'sed'，交互被定义为：忽略第NUMBERth个匹配项，然后匹配并替换所有从NUMBERth开始的匹配项。

# 5 sed正则表达式——选择文本
## 5.1 正则表达式概述
要知道如何使用“sed”，人们应该理解正则表达式（简称“regexp”）。正则表达式是一种从左到右与主题字符串进行匹配的模式。大多数字符都是“普通”字符：在一个模式中代表自己，并与相应的字符匹配。 “sed”中的正则表达式在两个斜杠之间指定。
以下命令打印包含单词“hello”的行：
...
正则表达式的功能来自在模式中包含替代和重复的能力。这通过使用“特殊字符”在模式中进行编码来实现，这些字符本身并不代表自己而是以某种特殊方式进行解释。

正则表达式中的字符“^”（插入符号）与行的开头匹配。 字符“.” （点）匹配任何单个字符。以下“sed”命令匹配并打印以字母“ b”开头，后跟一个任何单个字符，后跟字母“d”开头的行：
...
``` shell
	...
	body
```
以下各节说明正则表达式中特殊字符的含义和用法。

## 5.2 基本正则表达式与扩展正则表达式
基本和扩展正则表达式是特定模式语法的两个变体。基本正则表达式（BRE）语法是“sed”（和“grep”中类似）的默认设置。使用POSIX指定的“-E”选项（或“-r”，“--regexp-extended”）启用扩展正则表达式（ERE）语法。

在GNU “sed”中，基本正则表达式和扩展正则表达式之间的唯一区别在于一些特殊字符的行为：“?”，“+”，圆括号，花括号和“|”。

使用基本（BRE）语法时，这些字符没有特殊含义，除非在这些字符加上反斜杠（“\”）；使用扩展（ERE）语法时，则正好相反：这些字符有特殊含义，除非它们以反斜杠（'\'）开头。
|目标模式|BRE语法|ERE语法|
|:--|:--|:--|
|加号“+”| \$ echo 'a+b=c' > foo<br />\$ sed -n '/a+b/p' foo<br />a+b=c|\$ echo 'a+b=c' > foo <br />\$ sed -E -n '/a\+b/p' foo<br />a+b=c|
|一个或多个“a”字符，后跟“b”（加号作为特殊的元字符）|\$ echo aab > foo <br />\$ sed -n '/a\+b/p' foo<br />aab|\$ echo aab > foo<br />\$ sed -E -n '/a+b/p' foo<br />aab
