...
**-slave** 标志关闭与 spawn id 关联的 slave。 （参见“[spawn -pty](spawn_pty)”。）当连接关闭时，如果从属设备仍然打开，它也会自动关闭。

无论是隐式还是显式关闭连接，都应调用**wait**清除相应的内核进程插槽。 **close**不调用**wait**，因为不能保证关闭进程连接将导致close退出。请参阅下面的**wait**以获取更多信息。

-onexec 标志确定是否将在任何新生成的进程中关闭生成 ID，或者。 要使 spawn id 保持打开状态，请使用值 0。非零整数值将强制在任何新进程中关闭 spawn（默认值）。



   无论连接是隐式关闭还是显式关闭，都应该调用wait来清理相应的内核进程槽。 close 不会调用 wait，因为不能保证关闭进程连接会导致它退出。 请参阅下面的等待以获取更多信息。
