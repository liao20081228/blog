...
注意：打开双向套接字失败会导致向调用函数返回非致命错误。 如果在循环内使用管道、协进程或套接字到 **getline**，或者从 **print** 或 **printf**，则必须使用 **close**() 创建命令或套接字的新实例。 AWK 在返回 **EOF** 时不会自动关闭管道、套接字或协进程。

## 8.7 printf语句
AWK 版本的**printf** 语句和 **sprintf**() 函数（见下文）的 接受以下转换规范格式：
...
## 8.8 特殊文件名
<span id='SpecialFileNames'>当从</span> **print** 或 **printf** 到文件或通过文件的 getline 进行 I/O 重定向时，gawk 在内部识别某些特殊文件名。 这些文件名允许访问从 gawk 的父进程（通常是 shell）继承的打开文件描述符。 这些文件名也可以在命令行中用于命名数据文件。 文件名是：
