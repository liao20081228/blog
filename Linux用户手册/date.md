...
||--debug|注释解析的日期，并对有疑问的用法输出警告到stderr|
|-f DATEFILE| --file=DATEFILE|类似对于DATEFILE的每行都是执行一次--date|
|-I[FMT]|--iso-8601[=FMT]|以ISO 8601格式输出日期/时间。 FMT='date'仅用于日期（默认），'hours'，'minutes'，'seconds'，'ns'用于指定日期和时间的精度。示例：2006-08-14T02:34:56-06:00|
...
| -s STRING| --set=STRING|设定由STRING描述的时间|
...
在标志之后是可选的字段宽度，以十进制数表示；然后是一个可选的修饰符，如果可能，可以使用E来使用语言环境的替代表示形式，或者可以使用O来使用语言环境的替代数字符号。
