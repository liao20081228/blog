...
通常，在网络编程和RDMA编程中，一些操作涉及与远程对等方（例如地址解析和连接建立）和远程实体（例如路由解析和加入IB下的多播组）交互，其中通过IB动词管理的资源(如QP或AH)最终将在此交互中创建或实现。在这种情况下，其寻址语义是基于IP的应用程序可以使用与libibverbs协同工作的librdmacm。
