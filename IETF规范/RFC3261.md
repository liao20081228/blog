...
&emsp;&emsp;generic-message = start-line \*message-header CRLF [ message-body ] 
&emsp;&emsp;start-line = Request-Line / Status-Line
...
SIP-Version
: 请求和响应消息都包括正在使用的SIP版本，并遵循\[H3.1]（HTTP替换为SIP，HTTP/1.1替换为SIP/2.0）关于版本排序、合规性要求和版本号升级的规定。为了符合本规范，发送SIP消息的应用程序**必须**包括SIP版本：“SIP/2.0”。SIP版本字符串不区分大小写，但实现**必须**发送大写。
&emsp;
...
# 20 Header Fields 
头部字段的一般语法见第7.3节。本节列出了完整的头部字段集合以及有关语法、含义和用法的注释。在本节中，我们使用\[HX.Y]来引用当前HTTP/1.1规范RFC2616[^8]的第X.Y节。给出了每个头部字段的示例。

表2和表3总结了与方法和代理处理相关的头部字段信息。
```text
...
- R：标题字段只能出现在请求中；
- r：标题字段只能出现在响应中；
- 2xx、4xx等：数值或范围表示可以使用该头部字段的响应代码；
- c：头字段从请求复制到响应。
- “where”列中的空条目表示所有请求和响应中都可能存在该头部字段。
 
“proxy”列描述代理可以对头部字段执行的操作：
- a：代理可以添加或连接此头部字段（如果不存在）。
- m: 代理可以修改已存在的头部字段值。
- d: 代理可以删除头部字段值。
- r: 代理必须能够读取头部字段，因此此头部字段不能加密。

接下来的六列与方法中是否存在头部字段有关：
- c：有条件的；头部字段的要求取决于消息的上下文。
- m: 头部字段是必需的。
- m\*：头部字段**应该**被发送，但客户端/服务器需要准备好接收没有该头字段的消息。
- o: 头部字段是可选的。
- t：头部字段**应该**被发送，但客户端/服务器需要准备好接收没有该头字段的消息。

	如果使用基于流的协议（如TCP）作为传输，则必须发送头部字段。
- \*：如果消息正文不是空的，则头部字段是必需的。详见第20.14、20.15和7.4节。
- -：头部字段不适用。
 
“Optional”是指一个元素**可以**在请求或响应中包含此头部字段，UA**可以**忽略请求或响应中存在的此标题字段（该规则的例外是20.32中讨论的必需头部字段）。一个“mandatory”头部字段**必须**在请求中出现，并且接收请求的UAS**必须**理解该字段。一个“mandatory”响应头部字段**必须**出现在响应中，并且处理响应的UAC**必须**理解该字段。“Not applicable”表示请求中**不得**存在该字段。如果错误的放入“Not applicable”头部字段到一个请求中，则接收请求的UAS**必须**忽略它。类似地，标记为“Not applicable”的响应头部字段意味着UAS**不得**在响应中放置该字段，UAC**必须**忽略响应中的该字段。

UA**应当**忽略未理解的扩展头部参数。

还定义了一些常见头部字段名称的紧凑形式，以便在消息总大小出现问题时使用。

Contact、From和To头部字段包含URI。如果URI包含逗号、问号或分号，则URI**必须**用尖括号（\<和\>）括起来。任何URI参数都包含在这些括号内。如果URI未括在尖括号中，则任何分号分隔的参数都是头部参数，而不是URI参数。


## 20.1 Accept 
Accept头部字段遵循\[H14.1]中定义的语法。语义也相同，只是如果不存在Accept头部字段，服务器**应该**采用application/sdp的默认值。

空的Accept头部字段表示不接受任何格式。
...
Accept-Encoding头部字段类似于Accept，但限制响应中可接受的内容编码\[H3.5]。见\[H14.3]。SIP中的语义与\[H14.3]中定义的语义相同。

允许Accept-Encoding头部字段为空。它相当于接受编码：identity，也就是说，只允许使用identity编码，意味着没有编码。

如果不存在Accept-Encoding头部字段，则服务器**应当**采用identity的默认值。

这与HTTP定义略有不同，HTTP定义指出，当此字段不存在时，可以使用任何编码，但首选identity编码。
...
Accept-Language字段用于请求中，以指示响应中消息体的原因短语、会话描述或状态响应的首选语言。如果不存在Accept-Language头部字段，则服务器**应当**假定客户端可以接受所有语言。

Accept-Language头部字段遵循\[H14.4]中定义的语法。基于“q”参数对语言进行排序的规则也适用于SIP。
例如：
```
      Accept-Language: da, en-gb;q=0.8, en;q=0.7
```
## 20.4 Alert-Info 
当出现在INVITE请求中时，Alert-Info头部字段指定UAS的备选铃声。当出现在180（振铃）响应中时，Alert-Info头部字段指定UAC的备选回铃音。典型的用法是代理插入此头字段以提供独特的响铃特性。

Alert-Info头部字段可能会引入安全风险。第20.9节讨论了这些风险及其处理方法，其中讨论了Call-Info头部字段，因为这些风险是相同的。

此外，用户**应该**能够有选择地禁用此功能。

&emsp;&emsp;这有助于防止不受信任的元素使用此标头字段可能导致的中断。
...
```
## 20.5 Allow 
Allow头部字段列出生成消息的UA支持的方法集合。

UA理解的所有方法（包括ACK和CANCEL）都**必须**包含在Allow头部字段的方法列表中（如果存在）。如果没有Allow头部字段，则**不能**解释为发送消息的UA不支持任何方法。相反，这意味着UA没有提供任何关于其支持的方法的信息。

在对OPTIONS以外的方法的响应中提供Allow头部字段可以减少所需的消息数量。
...
Authentication-Info头部字段提供与HTTP摘要的相互认证。对于使用基于Authorization头部字段的摘要认证成功的请求，UAS可以在其2xx响应中携带该头域。
...
```
## 20.7 Authorization 
Authorization头部字段包含UA的身份验证凭据。第22.2节概述了Authorization头部字段的使用，第22.4节描述了与HTTP身份验证一起使用时的语法和语义。

此字段以及Proxy-Authorization打破了有关多个头部字段值的一般规则。尽管不是逗号分隔的列表，但此头部字段名称可以出现多次，并且**不得**使用第7.3节中描述的常规规则组合成单个标题行。
...
```
## 20.8 Call-ID 
Call-ID标头字段唯一标识特定邀请或特定客户端的所有注册。一次单独的多媒体会议可能会产生多个具有不同Call-ID的呼叫，例如，如果用户多次邀请单个用户参加同一（长期）会议。Call-ID区分大小写，只需简单地逐字节进行比较。
...
## 20.15 Content-Type 
Content-Type标头字段指示发送给收件人的消息正文的媒体类型。\[H3.7]中定义了“媒体类型”元素。如果正文不为空，则Content-Type字段必须存在。如果正文为空，并且存在Content-Type字段，则表示特定类型的正文长度为零（例如，空的音频文件）。
...
## 20.16 CSeq 
请求中的CSeq标头字段包含一个单独的十进制序列号和请求方法。序列号**必须**可以表示为32位无符号整数。CSeq的方法部分区分大小写。CSeq标头字段用于在对话中对事务进行排序，提供唯一标识事务的方法，并区分新请求和请求重传。如果序列号和请求方法相同，则认为两个CSeq标头字段相等。
...
有关如何对包含此标头字段的请求生成响应的详细信息，请参见第8.2.6节。尽管这里没有定义使用此标头字段的规范行为，但它允许用于扩展或SIP应用程序以获得RTT估计值。
...
响应码与HTTP/1.1响应码一致，并做了扩展。并非所有HTTP/1.1响应代码都是适当的，这里只给出了适当的。**不应**使用其他HTTP/1.1响应代码。此外，SIP定义了一个新的类别，6xx。

## 21.1 Provisional 1xx

临时响应（也称为信息性响应）表示所联系的服务器正在执行某些进一步的操作，并且还没有确定的响应。如果服务器预计需要200毫秒以上才能获得最终响应，则会发送1xx响应。请注意，1xx响应无法可靠传输。它们不会导致客户端发送ACK。临时响应（1xx）**可能**包含消息正文，包括会话描述。

### 21.1.1 100 Trying 

此响应表示下一个跃点服务器已接收到请求，并且正在代表此呼叫执行某些未指定的操作（例如，正在查询数据库）。与所有其他临时响应一样，此响应停止UAC对INVITE的重传。100（Trying）响应不同于其他临时响应，因为它永远不由有状态代理向上游转发。

### 21.1.2 180 Ringing 

接收到INVITE的UA正在尝试提醒用户。此响应**可能**用于启动本地回铃。
