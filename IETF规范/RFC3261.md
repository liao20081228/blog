...
# 14.1 UAC行为
适用于INVATE中的会话描述（第13.2.1节）的要约-应答模型同样适用于re-INVITE。因此，例如，想要添加媒体流的UAC将创建包含该媒体流的新要约，并在INVATE请求中将其发送给对等方。需要注意的是，发送的是会话的完整描述，而不仅仅是更改。这支持各种元素中的无状态会话处理，并支持故障切换和恢复功能。当然，UAC**可以**发送不带会话描述的重新邀请，在这种情况下，对re-INVITE的第一个可靠的非故障响应将包含要约（在本规范中，这是2xx响应）。
