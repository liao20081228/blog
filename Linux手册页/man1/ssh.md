...
ssh会连接到指定的<u>destination</u>主机并登录，目标主机可以指定为\[user@]hostname或ssh\://\[user@]hostname\[:port]形式的URI。用户必须使用以下几种方法之一向远程计算机证明自己的身份（见下文）。

如果指定了<u>command</u>，则该命令将在远程主机上执行，而不是启动登录shell。可以指定完整的命令行作为参数，也可以包含额外的参数。如果提供了这些参数，它们将被附加到命令后面，用空格分隔，然后一起发送给服务器执行。
