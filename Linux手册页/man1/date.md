...
|:--|:--|
|%%|文字％|
|%a|本地环境中的缩写工作日名称（例如，Sun）
|%A|本地环境中的完整工作日名称（例如，Sunday）
|%b|本地环境中的缩写月份名称（例如，Jan）
|%B|本地环境中的完整月份名称（例如，January）
|%c|本地环境中的日期和时间（例如，Thu Mar  3 23:05:25 2005）
...
|％p|相当于本地环境的AM或PM；如果未知，则为空白|
|％P|类似％p，但是小写|
|％q|季度（1..4）|
|％r|本地环境的12小时制时钟时间（例如，11:11:04 PM）
|％R|24小时制的时分；等同于％H:％M|
...
|％x|本地环境的日期表示形式（例如，12/31/99）|
|％X|本地环境的时间表示形式（例如，23:13:48）|
|％y|年份的后两位数字（00..99）|
|％Y|年|
|％z|+hhmm 数字时区（例如，**-0400**）|
...
在标志之后是可选的字段宽度，以十进制数表示；然后是一个可选的修饰符，E表示使用本地环境的替代表示形式，O表示使用本地环境的替代数字符号。
...
***<font color=blue>版权声明：</font>本文翻译自<font color=blue>《date date manpages》。</font>date的版本为8.30，手册更新时间为2019-09。<font color=red>本文与原始文档采用相同的版权许可。</font><font color=blue>转载请注明出处！！！</font>***
