...
**will** 指示由于某些代理采取不在本文件范围之内的行为，某些事情确定或者期待会发生。（**将来会**）

**will not** 指示由于某些代理采取不在本文件范围之内的行为，某些事情确定或者期待不会发生。（**将来不会**）

**might** 指示由于某些代理采取不在本文件范围之内的行为，某些事情可能会发生。（**可能会**）

**might not** 指示由于某些代理采取不在本文件范围之内的行为，某些事情可能不会发生。（**可能不会**）
...
## 8.1 IE类型
GTP控制平面（信令）消息允许包含多个IE。为了使GTPv2 IE具有向前兼容的类型定义，所有IE都应当采用TLIV（Type, Length, Instance, Value）编码。GTPv2 IE类型值在表8.1-1中指定。该表的最后一列指示IE是否为：
