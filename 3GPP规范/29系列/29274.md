...
名称以“Command”结尾的消息始终是初始消息。如果“Command”消息失败，则通过将“Command”替换为“Failure Indication”来构造应答消息的名称。除"Downlink Data Notification Failure Indication”消息外，“Failure Indication”为被触发消息。“Failure Indication”消息没有应答。如果“Command”消息成功，则其回复将是相关Stage 2程序中指定的Request。
...
- Delete Bearer Request/Response，在TAU/RAU/切换过程中，如果“ISR deactivation”原因值包含在Delete Session Request消息中，或者当它被发送以删除与另一个ISRAI标志未设置的ISRAI相关的CN节点上的承载资源时，删除承载请求/响应。
...
## 7.5 Non-3GPP接入相关消息
## 7.6 信号消息的可靠地传递
当前条款中的重传要求不适用于没有Triggered messages的Initial messages。

GTPv2消息的可靠传输是通过重传这些消息来实现的。如果消息需要回复，并且尚未收到回复，则应重传该消息。为了避免网络过载，可能会对重传次数进行限制。
