...
Piggybacking是可选特性，在3GPP TS 23.401 \[3]的附录F中描述。如果支持该特性，则用于EUTRAN初始附着、用于从可信或非可信非3GPP IP接入到E-UTRAN的切换（参见3GPP TS 23.402 \[45]第8.6和16.11条）、用于UE请求的PDN连接过程 的Triggered 响应中的初始消息的捎带确认应按照本规范第4.2.0和5.5.1条的要求实现。当使用捎带确认时，被触发的响应消息和捎带确认的初始消息应使用公共IP报头和公共UDP报头，如图4.2.0-2所示。紧随被触发响应消息之后是捎带确认的初始消息，在该消息之后不应出现其他信息。5.5条款规定了GTP-C头中piggybacking-specific字段的使用。
...
在SGGN池场景中， 如果Identification Request, Context Request 或 Suspend Notification消息

在SGSN池场景中，如果标识请求、上下文请求或挂起通知消息已经由池中的另一个SGSN转发，则标识响应、上下文响应或挂起确认消息的UDP目的端口应按以下方式确定。来自信息元素“UDP源端口号”的值（在相应的转发请求中发送）应复制到UDP目的端口字段。用于标识响应、上下文响应或暂停确认消息的UDP源端口可以是发送GTP实体处本地分配的端口号。
