...
在网络触发的业务恢复流程中（参见3GPP TS 23.007 \[17]），如果MME/S4-SGSN向SGW发送Downlink Data Notification Failure Indication消息。则该消息的目的地址应为通过下行链路数据通知消息中的用于控制平面的发送方F-TEID IE发送的SGW IP地址（如果存在于消息中），否则为先前接收的下行链路数据通知消息的源IP地址。
