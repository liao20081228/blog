...
搭便车的初始消息在被触发的响应消息之后作为联合体携带，它们共享一个公共的IP头部（参见图4.2.0-2）。
...
如果请求消息（例如，Create Session Request）触发了Piggybacking（即，Create Bearer Request搭便车在Create Session Response上），则请求消息的重传也应触发Piggybacking。Command消息使用的Sequence Number应将最高有效位设置为1。由Command消息触发的消息中的Sequence Number以及相应的Triggered Reply消息应与Command消息具有相同的Sequence Number（即，也应将最高有效位设置为1）。设置Sequence Number的最高有效位是为了避免Command消息选择的Sequence Number与GTPv2对等体为未由Command消息触发的Request消息选择的Sequence Number之间可能发生的冲突。
