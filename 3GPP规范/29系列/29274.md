...
- S4 SGSN在S16接口上接收到带有Cause值“Fallback to GTPv1”的GTPv2 Context Response消息。当UE通过S4 SGSN激活到GGSN的PDP上下文且SGSN间RAU正在进行时，旧的S4 SGSN应在S16接口上的GTPv2上下文响应消息中包含原因值“回退到GTPv1”。在这种情况下，新的S4 SGSN应中止正在进行的GTPv2过程并向旧的S4 SGSN发送GTPv1“SGSN上下文请求”消息。回退到GTPv1仅针对当前过程中的该UE执行。 
- MME在S3接口上接收到带有原因代码“回退到GTPv1”的GTPv2上下文响应消息。当UE通过S4 SGSN具有活动的PDP上下文且TAU正在进行时，旧的S4 SGSN可能在S3接口上的GTPv2上下文响应消息中包含原因值“回退到GTPv1”。在这种情况下，MME应中止正在进行的GTPv2过程并向旧的S4 SGSN发送GTPv1“SGSN上下文请求”消息。回退到GTPv1仅针对该UE执行。 
