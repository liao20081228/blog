...
除了附带的GTP消息或Echo Request消息外，如果GTP实体接收到一个IP/UDP数据包中的Request消息，其长度与GTP头中Length字段指定的值不一致，则接收的GTP实体应记录错误，并发送带有Cause IE值为“无效长度”的Response消息。 
除了附带的GTP消息外，如果GTP实体接收到一个IP/UDP数据包中的Response消息，其长度与GTP头中Length字段指定的值不一致，则接收的GTP实体应记录错误，并静默丢弃该消息。 
如果GTP实体接收到两个GTP消息（触发响应消息和附带的初始消息），其IP/UDP数据包的长度与两个连接消息的总长度（由GTP头的Length字段指示）不一致，则接收的GTP实体应记录错误，并返回带有Cause IE值为“触发响应消息和附带初始消息的总长度无效”的适当Response消息。即： 
- 对于Create Session Response消息和附带的Create Bearer Request消息，应返回带有上述Cause值的Create Bearer Response消息。 
- 对于Create Bearer Response消息和附带的Modify Bearer Request消息，应返回带有上述Cause值的Modify Bearer Response消息。
