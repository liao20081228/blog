...
>注1：在Create Session Request或Modify Bearer Request中通告单个IP地址类型可确保GTP-C对等体在与该GTP-C隧道相关的反向方向上的后续控制面初始消息中使用的IP地址类型无歧义。在建立GTP-C隧道过程中，或者在两个GTP-C对等体之间建立通信过程中，避免了IP地址的随意切换。

>注2：在具有不同IPv6能力的MME/SGSN或SGW的部署中，当MME/SGSN或SGW发生变化时，可能会发生IPv4和IPv6之间的IP切换。

>示例1：如果MME从DNS获得SGW的IPv4地址，则MME仅在Create Session Request的Sender F-TEIDIE中包含IPv4地址。在响应中，SGW通告IPv4地址和可选的IPv6地址，并且SGW在其发送到与该GTP-C隧道相关的MME的后续控制平面Initial消息中使用IPv4寻址。

>示例2：作为示例1的继续，在MME的后续更改时，假设源MME仅支持IPv4且目标MME支持IPv4和IPv6，则目标MME可以通过使用IP报头中的SGW S11 IPv6地址并包括发送方F向SGW发送修改承载请求来切换到IPv6编址仅具有MME S11 IPv6地址的TEID。
