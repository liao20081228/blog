...
### 7.7.9 未知或意外IE
接收包含已知类型值但实例值未定义的意外信息元素的GTP消息的接收方应丢弃该信息元素并记录错误。接收方应处理该消息。
根据本规范第7条定义的语法，在接收到的GTP信令消息中，类型值在本规范第8.1条中定义但实例值未预期的信息元素应被静默丢弃（跳过），消息的其余部分应像该信息元素不存在一样进行处理。
注意：编码的GTPv2消息或分组信息元素中的信息元素由信息元素类型和实例值对标识。
### 7.7.10 重复IE
