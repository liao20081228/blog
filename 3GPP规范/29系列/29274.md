...
GTP实体应检查接收到的没有拒绝Cause值的Response消息中是否包含所有mandatory IE。如果缺少一个或多个mandatory IE，GTP实体应通知上层并记录错误。如果Response消息中缺少mandatory IE，而SGW需要通过另一个接口转发（例如，当SGW从PGW转发消息到MME时），SGW应在转发的响应消息中包含拒绝原因“来自远程对等方的无效回复”（参见第8.4条）。 

GTP实体应检查接收到的请求消息中是否存在条件性信息元素，如果可能的话（即接收实体有足够的信息来检查相应条件是否满足）。如果缺少一个或多个条件性信息元素，GTP实体应记录错误，并发送带有“Conditional IE missing”原因IE值的响应消息，同时包含缺失的条件性IE的类型和实例。 

GTP实体应检查接收到的响应消息中是否存在条件性信息元素，且没有拒绝原因值，如果可能的话（即接收实体有足够的信息来检查相应条件是否满足）。如果缺少一个或多个条件性信息元素，GTP实体应通知上层并记录错误。 

对于包含拒绝原因值的响应消息，参见第6.1.1条。 

如果指示IE作为条件性IE适用于该消息且不存在，除非有其他原因拒绝该消息，否则GTP实体不应拒绝该消息。 

如果指示IE作为条件性IE适用于该消息且存在，且所有适用的标志值都设置为“0”，除非有其他原因拒绝该消息，否则GTP实体不应拒绝该消息。 

可选信息元素的缺失不应触发任何错误处理过程。
### 7.7.7 无效的IE长度
