...
>注3：假设配置为通过non-3GPP接入从5GC接收服务的UE，在non-3GPP接入上处于RM-DEREGISTERED或CM-IDLE状态时，只要成功连接到non-3GPP接入网络，除非网络禁止建立Non-3GPP Access Connection（例如由于网络拥塞），否则将尝试建立Non-3GPP Access
Connection并转换到CM-CONNECTED状态。
