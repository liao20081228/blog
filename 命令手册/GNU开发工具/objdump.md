...
该选项允许为存放无歧义类型的 CTF 父字典单独指定一个段，与存放剩余歧义类型的子字典段区分开。 标准链接器不会生成这种结构的 ELF 文件，但部分第三方链接器可能会这么做。 同时该选项也便于查看以多个原始文件形式写出、并通过 `objcopy` 合并的 CTF 数据——这类数据会被放入不同 ELF 段，而非同一个 CTF 字典的不同成员中。 

- sframe[=section] 显示指定 SFrame 段的内容。 默认显示名为 `.sframe` 的段，该名称由链接器 `ld` 生成。 
- -G / --stabs 显示所有请求段的完整内容。 从 ELF 文件中输出 `.stab`、`.stab.index` 和 `.stab.excl` 段的内容。 该选项仅在部分系统（如 Solaris 2.0）上有用，这类系统会将 `.stab` 调试符号表条目存放在 ELF 段中。 在大多数其他文件格式中，调试符号表条目与链接符号交错存放，可通过 `--syms` 查看。
- --start-address=address 从指定地址开始显示数据。 该选项会影响 `-d`、`-r` 和 `-s` 选项的输出。 
- --stop-address=address 在指定地址处停止显示数据。 该选项会影响 `-d`、`-r` 和 `-s` 选项的输出。
