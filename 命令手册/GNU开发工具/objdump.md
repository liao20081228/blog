...
- -T, --dynamic-syms
打印文件的动态符号表条目。该参数仅对动态目标文件有效，例如各类共享库。其输出信息与 `nm` 工具携带 `-D`（`--dynamic`）选项时展示的内容相近。 输出格式和 `--syms` 选项的输出大体一致，区别是会在符号名称前新增一列字段，用于展示该符号关联的版本信息。 若该版本是解析无版本标记引用时默认使用的版本号，则直接原样显示；其余版本号会放置在圆括号内输出。
### --special-syms
