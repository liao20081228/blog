...
- -G / --stabs 
显示所有请求节的完整内容。 从 ELF 文件中输出 `.stab`、`.stab.index` 和 `.stab.excl` 段的内容。 该选项仅在部分系统（如 Solaris 2.0）上有用，这类系统会将 `.stab` 调试符号表条目存放在 ELF 段中。 在大多数其他文件格式中，调试符号表条目与链接符号交错存放，可通过 `--syms` 查看。
