...
向反汇编器传递与目标平台相关的特定信息。该功能仅在部分目标平台上受支持。若需要指定多个反汇编选项，可多次使用 `-M` 参数，或将多个选项以逗号分隔的形式合并书写。
	...
	针对 VAX 架构 可通过 `-M entry:0xf00ba` 指定函数入口地址，可多次使用该参数。 该功能用于正确反汇编不含符号表的 VAX 二进制文件（如 ROM 转储文件）。 若无此设置，函数入口掩码会被误解析为 VAX 指令，导致函数后续代码反汇编出错。
