...
如果 CTF 节包含存在歧义的类型定义，它会以归档形式包含多个 CTF 字典，所有字典均继承自一个包含无歧义类型的基础字典。 该基础成员默认名为 `.ctf`，与所在段同名；但在链接时可通过 `ctf_link_set_memb_name_changer` 函数修改此名称。 当查看由使用了该重命名功能的链接器生成的 CTF 归档时，可通过 `--ctf-parent` 指定基础父成员的名称。 

- --ctf-parent-section=section 该选项允许为存放无歧义类型的 CTF 父字典单独指定一个段，与存放剩余歧义类型的子字典段区分开。 标准链接器不会生成这种结构的 ELF 文件，但部分第三方链接器可能会这么做。 同时该选项也便于查看以多个原始文件形式写出、并通过 `objcopy` 合并的 CTF 数据——这类数据会被放入不同 ELF 段，而非同一个 CTF 字典的不同成员中。 
