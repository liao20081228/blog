...
打印文件中的符号表条目。尽管显示格式不同，但其展示的信息与 `nm` 程序类似。输出格式取决于被转储文件的格式，主要分为两大类。 一类格式如下所示：
	...
	节名之后是一个数字字段：对于公共符号，该值表示对齐要求；对于其他符号，则表示大小。最后一列是符号名称。 标志字符共分为 7 组，含义如下： - **`l` / `g` / `u` / `!`** 符号为本地符号（`l`）、全局符号（`g`）、唯一全局符号（`u`）、既非全局也非本地（空格），或既是全局又是本地（`!`）。 符号既非本地也非全局可能出于多种原因（例如用于调试），但如果同时标记为本地和全局，通常意味着文件存在错误。 唯一全局符号是 GNU 对标准 ELF 符号绑定的扩展，动态链接器会保证整个进程中仅有一个该名称和类型的符号被使用。 - **`w`** 符号为弱符号（`w`）或强符号（空格）。 - **`C`** 符号为构造函数符号（`C`）或普通符号（空格）。 - **`W`** 符号为警告符号（`W`）或普通符号（空格）。若后续符号被引用，警告符号的名称会作为提示信息打印出来。 - **`I` / `i`** 符号是对另一符号的间接引用（`I`）、需在重定位阶段求值的函数（`i`），或普通符号（空格）。 - **`d` / `D`** 符号为调试符号（`d`）、动态符号（`D`）或普通符号（空格）。 - **`F` / `f` / `O`** 符号为函数名（`F`）、文件名（`f`）、对象名（`O`）或普通符号（空格）。
