...
- @<u>file</u>
从文件<u>file</u>中读取命令行选项。读取到的选项会替换原有的 <u>@file</u> 选项。 如果文件不存在或无法读取，则该选项会按原样保留，不做处理。文件中的选项以空白字符分隔。若某个选项中需要包含空白字符，可将整个选项用单引号或双引号包裹。 任意字符（包括反斜杠）均可通过在其前添加反斜杠进行转义。 文件内也可包含更多 @file 选项，此类选项会被递归处理。
