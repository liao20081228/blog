...
向反汇编器传递与目标平台相关的特定信息。该功能仅在部分目标平台上受支持。若需要指定多个反汇编选项，可多次使用 `-M` 参数，或将多个选项以逗号分隔的形式合并书写。
	...
	针对 RISC-V 架构 支持以下选项： 
	`max`：直接反汇编而不检查架构字符串。这是一种尽力而为的模式，对存在重叠的指令集扩展，将采用首个匹配项（在特定上下文下可能并不正确）来对指令进行解码。如果 ELF 文件未暴露 ISA 字符串，导致无法自动推导指令集子集，且默认回退的 ISA 字符串（`rv64gc`）无法覆盖二进制文件中的所有指令时，该模式会非常有用。 
