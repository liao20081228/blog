...
如果 CTF 节包含存在歧义的类型定义，它会以归档形式包含多个 CTF 字典，所有字典均继承自一个包含无歧义类型的基础字典。 该基础成员默认名为 `.ctf`，与所在节同名；但在链接时可通过 `ctf_link_set_memb_name_changer` 函数修改此名称。 当查看由使用了该重命名功能的链接器生成的 CTF 归档时，可通过 `--ctf-parent` 指定基础父成员的名称。 
