...
只从编号为 <u>n</u> 的调试信息条目（DIE）开始打印。该选项仅与 `--debug-dump=info` 配合使用时有效。 
	...
	若指定该选项，将不再输出任何头部信息，以及编号为 <u>n</u> 的条目之前的所有 DIE。仅打印指定条目及其同级、下级子条目。 本选项可与 `--dwarf-depth` 配合使用。 --- ### --dwarf-check 启用额外检查，校验 DWARF 调试信息的一致性。 --- ### --ctf[=section] 显示指定 CTF 段的内容。CTF 段本身包含多个子段，所有子段会按顺序显示。 默认显示名为 `.ctf` 的段，该名称由链接器 `ld` 生成。 --- ### --ctf-parent=member 如果 CTF 段包含存在歧义的类型定义，它会以归档形式包含多个 CTF 字典，所有字典均继承自一个包含无歧义类型的基础字典。 该基础成员默认名为 `.ctf`，与所在段同名；但在链接时可通过 `ctf_link_set_memb_name_changer` 函数修改此名称。 当查看由使用了该重命名功能的链接器生成的 CTF 归档时，可通过 `--ctf-parent` 指定基础父成员的名称。 --- ### --ctf-parent-section=section 该选项允许为存放无歧义类型的 CTF 父字典单独指定一个段，与存放剩余歧义类型的子字典段区分开。 标准链接器不会生成这种结构的 ELF 文件，但部分第三方链接器可能会这么做。 同时该选项也便于查看以多个原始文件形式写出、并通过 `objcopy` 合并的 CTF 数据——这类数据会被放入不同 ELF 段，而非同一个 CTF 字典的不同成员中。 --- ### --sframe[=section] 显示指定 SFrame 段的内容。 默认显示名为 `.sframe` 的段，该名称由链接器 `ld` 生成。 --- ### -G / --stabs 显示所有请求段的完整内容。 从 ELF 文件中输出 `.stab`、`.stab.index` 和 `.stab.excl` 段的内容。 该选项仅在部分系统（如 Solaris 2.0）上有用，这类系统会将 `.stab` 调试符号表条目存放在 ELF 段中。 在大多数其他文件格式中，调试符号表条目与链接符号交错存放，可通过 `--syms` 查看。 --- ### --start-address=address 从指定地址开始显示数据。 该选项会影响 `-d`、`-r` 和 `-s` 选项的输出。 --- ### --stop-address=address 在指定地址处停止显示数据。 该选项会影响 `-d`、`-r` 和 `-s` 选项的输出。
