...
- -W \[lLiaprmfFsoORtUuTgAckK], --dwarf \[=rawline,=decodedline,=info,=abbrev,=pubnames,
...
`"K"` `"=follow-links"`：显示在链接的独立调试信息文件中找到的所有选定调试节的内容。若同一调试节在多个文件中存在，可能会重复显示。 
此外，在显示 DWARF 属性时，若遇到引用独立调试信息文件的格式，也会一并显示被引用的内容。
注意：在部分发行版中，该选项默认启用，可通过 `N` 调试选项禁用。在配置 binutils 时，可通过 `--enable-follow-debug-links=yes` 或 `--enable-follow-debug-links=no` 选择默认行为；若未指定该配置项，则默认启用调试链接追踪。 
注意：若编译 binutils 时启用了对 debuginfod 协议的支持，该选项还会尝试连接 `DEBUGINFOD_URLS` 环境变量中指定的 debuginfod 服务器，此过程可能耗时较长。可通过 `=do-not-use-debuginfod` 调试选项禁用该行为。
...
- --dwarf-depth=<u>n</u>
将 `.debug_info` 段的转储内容限制为 <u>n</u> 级子节点。 该选项仅与 `--debug-dump=info` 配合使用时有效。 默认行为是打印所有调试信息条目（DIE）；若将 <u>n</u> 设为特殊值 0，效果与此相同。 
	当 n 为非零值时，深度等于或大于 n 级的 DIE 将不会被打印。 <u>n</u> 的取值从 0 开始计数。

- --dwarf-start=<u>n</u>
只从编号为 <u>n</u> 的调试信息条目（DIE）开始打印。该选项仅与 `--debug-dump=info` 配合使用时有效。 

	若指定该选项，将不再输出任何头部信息，以及编号为 <u>n</u> 的条目之前的所有 DIE。仅打印指定条目及其同级、下级子条目。 
	
	本选项可与 `--dwarf-depth` 配合使用。 

- --dwarf-check
启用额外检查，校验 DWARF 调试信息的一致性。 
- --ctf\[=section]
显示指定 CTF 节的内容。CTF 节本身包含多个子节，所有子节会按顺序显示。 

	默认显示名为 `.ctf` 的段，该名称由链接器 `ld` 生成。 
- --ctf-parent=<u>member</u> 
如果 CTF 节包含存在歧义的类型定义，它会以归档形式包含多个 CTF 字典，所有字典均继承自一个包含无歧义类型的基础字典。 该基础成员默认名为 `.ctf`，与所在节同名；但在链接时可通过 `ctf_link_set_memb_name_changer` 函数修改此名称。 当查看由使用了该名称修改器重命名父归档成员的链接器生成的 CTF 归档时，可通过 `--ctf-parent` 指定父成员的名称。 

- --ctf-parent-section=<u>section</u> 
该选项允许你为存放无歧义类型的 CTF 父字典指定一个完全独立的节，与存放剩余歧义类型的子字典所在节区分开。 标准链接器不会生成这种结构的 ELF 目标文件，但部分第三方链接器可能会这么做。 同时，该选项也便于查看以多个原始文件形式输出、并通过 `objcopy` 合并的 CTF 数据——`objcopy` 可以将这些数据放入不同的 ELF 段，但无法将它们放在同一个 CTF 字典的不同成员中。

- sframe\[=<u>section</u>]
显示指定 SFrame 节的内容。 

	默认显示名为 `.sframe` 的段，该名称由链接器 `ld` 生成。 

- -G / --stabs 
显示所有请求节的完整内容。 从 ELF 文件中输出 `.stab`、`.stab.index` 和 `.stab.excl` 节的内容。 该选项仅在部分系统（如 Solaris 2.0）上有用，这类系统会将 `.stab` 调试符号表条目存放在 ELF 段中。 在大多数其他文件格式中，调试符号表条目与链接符号交错存放，可通过 `--syms` 查看。
 
- --start-address=<u>address</u> 
从指定地址开始显示数据。 该选项会影响 `-d`、`-r` 和 `-s` 选项的输出。 
- --stop-address=<u>address</u> 
在指定地址处停止显示数据。 该选项会影响 `-d`、`-r` 和 `-s` 选项的输出。

- -t, --syms
打印文件中的符号表条目。尽管显示格式不同，但其展示的信息与 `nm` 程序类似。输出格式取决于被转储文件的格式，主要分为两大类。 一类格式如下所示：
	```
	[ 4](sec 3)(fl 0x00)(ty 0)(scl 3) (nx 1) 0x00000000 .bss 
	[ 6](sec 1)(fl 0x00)(ty 0)(scl 2) (nx 0) 0x00000000 fred
	``` 
	其中方括号内的数字是符号表中的条目编号；`sec` 为节编号；`fl` 是符号的标志位；`ty` 是符号类型；`scl` 是符号存储类别；`nx` 是该符号关联的辅助条目数量。最后两字段分别是符号的值和符号名称。
	
	另一种常见输出格式（多见于 ELF 格式文件）如下： 
	``` 
	00000000 l d .bss 00000000 .bss 
	00000000 g .text 00000000 fred 
	``` 
	第一个数字是符号的值（有时也称为地址）。 下一列是一组字符与空格，代表符号上设置的标志位（具体字符含义见下文）。 再往后为符号所属的节，或为 `*ABS*` 表示绝对符号（不归属任何节）；或为 `*UND*` 表示该符号在当前文件中被引用但未定义。
	
	节名之后是一个数字字段：对于公共符号，该值表示对齐要求；对于其他符号，则表示大小。最后一列是符号名称。
	
	标志字符共分为 7 组，含义如下： 
	
	`l` / `g` / `u` / `!`：符号为本地符号（`l`）、全局符号（`g`）、唯一全局符号（`u`）、既非全局也非本地（空格），或既是全局又是本地（`!`）。 符号既非本地也非全局可能出于多种原因（例如用于调试），但如果同时标记为本地和全局，通常意味着文件存在错误。 唯一全局符号是 GNU 对标准 ELF 符号绑定的扩展，对于这样的符号，动态链接器会保证整个进程中仅有一个该名称和类型的符号被使用。
	
	`w`： 符号为弱符号（`w`）或强符号（空格）。
	`C`：符号为构造函数符号（`C`）或普通符号（空格）。 
	`W`：符号为警告符号（`W`）或普通符号（空格）。若后续符号被引用，警告符号的名称会作为提示信息打印出来。 
	`I` / `i`： 符号是对另一符号的间接引用（`I`）、需在重定位阶段求值的函数（`i`），或普通符号（空格）。 
	`d` / `D`：符号为调试符号（`d`）、动态符号（`D`）或普通符号（空格）。 
	`F` / `f` / `O`： 符号为函数名（`F`）、文件名（`f`）、对象名（`O`）或普通符号（空格）。

- -T, --dynamic-syms
打印文件的动态符号表条目。该参数仅对动态目标文件有效，例如各类共享库。其输出信息与 `nm` 工具携带 `-D`（`--dynamic`）选项时展示的内容相近。 

	输出格式和 `--syms` 选项的输出大体一致，区别是会在符号名称前新增一列字段，用于展示该符号关联的版本信息。 若该版本是解析无版本标记引用时默认使用的版本号，则直接原样显示；其余版本号会放置在圆括号内输出。
	
- --special-syms
显示符号时，包含目标平台认定为具有某种特殊性、且通常用户不会关心的那些符号。

- -U \[d|i|l|e|x|h], --unicode=\[default|invalid|locale|escape|hex|highlight]
控制字符串中UTF-8 编码多字节字符的显示方式。 默认设置（`--unicode=default`）是不对其做特殊处理。 `--unicode=locale` 选项会按照当前系统区域设置显示字符序列，但当前区域设置不一定支持这些 Unicode 字符。 `--unicode=hex` 和 `--unicode=invalid` 选项会将其以十六进制字节序列形式展示，分别用尖括号或花括号括起。 
...
- -x, --all-headers
显示所有可用的头部信息，包括符号表和重定位条目。 使用 `-x` 等价于同时指定 `-a -f -h -p -r -t` 全部选项。

- -w, --wide
为列数超过 80 列的输出设备对部分行进行格式化显示，同时在展示符号名称时不进行截断。

- -z, --disassemble-zeroes
通常反汇编输出会跳过全零数据块。该选项指示反汇编器将这些块与其他数据一样正常进行反汇编。。
- -Z, --decompress
`-Z` 选项需与 `-s` 选项配合使用。它指示 objdump 在显示压缩段内容前先对其进行解压缩。

- @<u>file</u>
从文件<u>file</u>中读取命令行选项。读取到的选项会替换原有的 @<u>file</u> 选项。 如果文件<u>file</u>不存在或无法读取，则该选项会按字面量处理，不做移除。

	文件<u>file</u>中的选项以空白字符分隔。若某个选项中需要包含空白字符，可将整个选项用单引号或双引号包裹。 任意字符（包括反斜杠）均可通过在其前添加反斜杠进行转义。 文件<u>file</u>内也可包含更多 @<u>file</u> 选项，此类选项会被递归处理。
