...
仅将指定符号 <u>symbolname</u> 保留为全局符号，将其他所有符号本地化到文件内，使其对外不可见。该选项可多次使用。 注意：此选项不能与 `--globalize-symbol` 或 `--globalize-symbols` 选项同时使用。

- --localize-hidden 
在 ELF 目标文件中，将所有具备隐藏可见性或内部可见性的符号标记为局部符号。该选项会叠加在 `-L` 这类针对特定符号的本地化选项之上生效。
