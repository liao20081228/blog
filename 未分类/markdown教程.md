...
Markdown 的段落是由一个或多个连续的文本行组成，它的前后要有一个以上的空行（空行的定义是显示上看起来像是空的，便会被视为空行。比方说，若某一行只包含空格和制表符，则该行也会被视为空行）。普通段落不该用空格或制表符来缩进，而应使用html语法。
...
**在引用里可以使用其他 markdown 语法，例如：标题、列表、粗体、斜体、图片、链接、表格、代码、代码块、高级代码块等，对于需要多行的语法每一行都要添加相同数量的`<`标记**。
...
**无序列表可以有多级，每一级用一个缩进表示：两个空格、三个空格或一个制表符，同级连续列表项应使用相同数量的同一缩进符号**。例如：
...
**单个列表项可以有多段，每段之间用空行隔开，每段第一行或每一行必须在列表项目的缩进基础上再加一个缩进：两个空格、三个空格或一个制表符**。例如：
...
**一个列表项可以嵌套引用，如果引用作为列表项内容的一段，则应该在列表项的缩进基础上加上一个缩进，即两个空格、三个空格或一个制表符**。例如：
...
*     嵌套代码块作为列表项（1空格+4个空格）
      嵌套代码块作为列表项（6个空格，2个表示属于列表项+4个表示代码块）
        嵌套代码块作为列表项（8个空格，4个表示属于列表项+2个表示代码块+2个表示空格）
		嵌套代码块作为列表项（两个制表符，1个表示制表符+1个表示代码块）
		嵌套代码块作为列表项（两个制表符）
...
用`<span id='idname'>跳转文字</span>`设置锚点，在其他地方就可以使用`[跳转处](#idname)` 进行跳转。例如：[跳转到1.13](#test)。
...
任务列表,`- [] 任务`表示未完成，`- [X] 任务`或`- [x] 任务`表示已完成， 任务列表也支持多级使用。例如：
...
**小书匠支持代码使用`>>对应的文字格式语法<<`来设置代码块内文字格式：高亮、下划线、加粗、删除线等。**


**小书匠支持使用`语法关键字?linenumber=true或false`来开启或关闭行号。**
...
**引用脚注**: `内容[^id] ` ，然后在任意行首使用`[^id]:脚注内容`。显示效果：内容[^id]

[^id]:脚注内容

**内联脚注**：`正文内容^[脚注定义内容]`，显示效果：正文内容^[脚注定义内容]

**多行或多段脚注**：允许在脚注定义里，进行多行定义或多段，每行或每段需要一个缩进，即至少两个空格或者一个制表符。只有引用式脚注支持多行脚注内容。例如：
...
行内数学公式：`$数学公式$`，如` $a^2+b^2 $`，显示效果：$a^2+b^2$；
块级数学公式：`$$数学公式$$`，如`$$a^2+b^2 =c^2$$`，显示效果：$$a^2+b^2 =c^2$$

具体的语法参考<https://katex.org/docs/supported.html>。
