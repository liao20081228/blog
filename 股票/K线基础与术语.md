...
自由流通股是在流通股本的基础上剔除了以下项目：

- 持股5%以上（含5%）的公司创始人（创始人家族）持股；
- 持股5%以上（含5%）的高管持股；
- 持股5%以上（含5%）的国有股东持股；
- 持股5%以上（含5%）的冻结股份；
- 持股5%以上（含5%）的战略投资者股份；
...
每一笔成交明细，实际上是过去3秒的时间内成交的一个数据包，而不是显示的每一次成交。

成交明细格式：`时间 价格 涨跌 买卖 总量 笔数`。
- 时间：每隔3秒才刷新一次。
- 价格：最后一笔成交价格。
- 涨跌：相对上一个成交明细的成交价格的变化。
- 买卖：最后一笔成交为主卖还是主卖。
- 总量：三秒内的成交股票手数。
- 笔数：三秒内的成交笔数。其中上海交易所的股票会显示，而深圳交易所则没有。

## 逐笔成交

类似成交明细，但是为实时推送的。level2功能。逐笔成交每秒从1开始递增序号。显示每一笔成交的价格、涨跌标记、数量。
